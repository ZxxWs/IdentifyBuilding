    ...
    for label, confidence, bbox in detections:
        ...
        if coordinates:
            print("{}: {}%    (left_x: {:.0f}   top_y:  {:.0f}   width:   {:.0f}   height:  {:.0f})".format(label,
                                                                                                            confidence,
                                                                                                            x, y, w, h))
            returnTXT += "{}: {}%    (left_x: {:.0f}   top_y:  {:.0f}   width:   {:.0f}   height:  {:.0f})".format(
                label,
                confidence,
                x, y, w, h)
        else:
            print("{}: {}%".format(label, confidence))
            returnTXT += "{}: {}%".format(label, confidence)
        return returnTXT
